...
union({error, _}, D) -> D;
union(D, {error, _}) -> D;
union({sum, D1}, {sum, D2}) -> {sum, maps:merge(D1, D2)};
union({sum, D1}, D) -> {sum, maps:put(D, true, D1)};
union(D, {sum, D1}) -> union({sum, D1}, D);
union({tagged, Tag, D1}, {tagged, Tag, D2}) -> {tagged, Tag, union(D1, D2)};
union({product, D1}, {product, D2}) -> 
    {sum, #{{product, D1} => true, {product, D2} => true}};
union({f, Name1, F1}, {f, Name2, F2}) -> 
    ...
    case {domain_util:get_arity(F1), domain_util:get_arity(F2)} of
        ...
        _ -> {sum, #{{f, Name1, F1} => true, {f, Name2, F2} => true}}
    end;
union(D1, D2) -> {sum, #{D1 => true, D2 => true}}.
